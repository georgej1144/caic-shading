...
class FORECAST_INTERPRETER {
    ...
    find_region_for_latlon(lat,lon, regions) {
        ...
        for (const region of regions) {
            if (typeof region !== 'object' || 
                ...
                (region.geometry.type !== 'MultiPolygon' && region.geometry.type !== 'Polygon')) {
            ...
            try {
                if(region.geometry.type === 'MultiPolygon') {
                    var polygon = turf.multiPolygon(region.geometry.coordinates); // Create Turf polygon
                } else if (region.geometry.type === 'Polygon') {
                    var polygon = turf.polygon(region.geometry.coordinates);
                }
    ...
    sort_trim_aspects(aspects) {
        ...
        const octantOrder = Object.keys(this.aspect_mapping);

        const indices = aspects.map(asp => octantOrder.indexOf(asp));
        const sorted = indices.sort((a, b) => a - b);

        // Handle wrap-around: if the run crosses north (e.g. northwest, north, northeast)
        // the sorted indices will be non-contiguous like [0, 1, 6, 7]
        // In that case, the true start is the first index after the gap
        const startsAfterGap = sorted.findIndex((val, i) =>
            i > 0 && val > sorted[i - 1] + 1
        );

        const startIdx = startsAfterGap !== -1 ? sorted[startsAfterGap] : sorted[0];
        const endIdx   = startsAfterGap !== -1 ? sorted[startsAfterGap - 1] : sorted[sorted.length - 1];

        const startDeg = this.aspect_mapping[octantOrder[startIdx]][0];
        const endDeg   = this.aspect_mapping[octantOrder[endIdx]][1];
    ...
    format_as_rule(aspect_range, elevation_range, color, options) {
        let ret = this.rule_tool("s", [options.slideSlopeMin, options.slideSlopeMax]);
    ...
    danger_to_rule(problem, options) {
        ...
        for(const key of Object.keys(aspects)) {
            ...
            if(aspects[key]) {
                ...
                rule.title = " " + problem.name + "_" + options.date;
    ...
    async run(options) {
        let problems = await this.get_problems_from_coords(options.lat, options.lon);
        if(!problems) {
            // no problems found in forecast
            //TODO: tell user what happened
            console.warn('no problems?', problems);
            return;
        }
        console.log(problems)
        return this.interpret_problems_to_DEM(problems, options);
...
export class NAC_API extends FORECAST_INTERPRETER {
    ...
    async avy_regions() {
        ...
        try {
            const resp = await this.getJsonFromEndpoint(data);
            let ret = [];
    ...
    async get_problems_from_coords(lat,lon) {
        ...
        regions = await this.avy_regions();
        let result = this.find_region_for_latlon(lat,lon,regions);
        center_id = result[0];
        zone_id = result[1];
        console.log(result, center_id, zone_id);
        forecast = await this.avy_forecast(center_id, zone_id);
